...
open class LangKotlinTemplates {
    ...
    open fun dslBuilderI(nameBuilder: TemplateI<CompilationUnitI>.(CompilationUnitI) -> NamesI = defaultNameBuilder)
            = Template<CompilationUnitI>("BuilderI", nameBuilder) { item, c -> item.toKotlinDslBuilderI(c) }

    open fun dslObjectTree(nameBuilder: TemplateI<CompilationUnitI>.(CompilationUnitI) -> NamesI = defaultNameBuilder)
    ...
    open fun enumParseMethod(nameBuilder: TemplateI<EnumTypeI>.(CompilationUnitI) -> NamesI = defaultNameBuilder)
