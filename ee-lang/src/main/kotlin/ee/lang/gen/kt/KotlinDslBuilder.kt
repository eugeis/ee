...
fun <T : AttributeIB<*>> T.toKotlinDslBuilderMethods(c: GenerationContext, derived: String, api: String,
    ...
    ${override}fun ${name()}(vararg $value: ${toKotlinTypeSingleB(c, api)}): B = apply { ${name()}().addItems(value.asList()) }"""
    }, {
        """
    ...
    ${override}fun ${name()}($value: ${toKotlinDslTypeDef(c, api)}): B = apply { attr(${
        ...
        """
    ${override}fun ${nonFluent()}($value: ${toKotlinTypeSingleB(c, api)}): ${toKotlinTypeSingleB(c, api)} = applyAndReturn { ${multi().ifElse({
            """${name()}().addItem($value); value"""
        }, { """${name()}().value($value)""" })} }
    ${override}fun ${nonFluent()}($value: ${toKotlinTypeSingleB(c, api)}.() -> Unit): ${toKotlinTypeSingleB(c, api)} = ${nonFluent()}(${toKotlinTypeSingle(c, derived)}($value))"""
    }}"""
}
...
open class $B<B : $B<B>>(value: B.() -> Unit = {}) : ${c.n(superUnit(), LangDerivedKind.MANUAL)}<B>(value)${(derived != api).then(
            { ", ${c.n(this, LangDerivedKind.API)}<B>" })} {${
