    ...
    return when (baseType) {
        ...
        n.List -> (attr.isNotEMPTY() && attr.mutable().setAndTrue()).ifElse("new Array()", "new Array()")
            ...
            if (baseType is Literal) {
                "${(baseType.findParent(EnumTypeI::class.java) as EnumTypeI).toTypeScript(c, derived, attr)}.${baseType.toTypeScript()}"
...
fun GenericI.toTypeScript(c: GenerationContext, derived: String): String =
        c.n(type(), derived)
...
fun <T : TypeI> T.toTypeScript(c: GenerationContext, derived: String, attr: AttributeI = Attribute.EMPTY): String =
...
fun <T : AttributeI> T.toTypeScriptInitMember(c: GenerationContext, derived: String): String = "this.${name()}${toTypeScriptInit(c, derived)}"
fun <T : AttributeI> T.toTypeScriptSignature(c: GenerationContext, derived: String, api: String, init: Boolean = true): String =
...
fun <T : ConstructorI> T.toTypeScript(c: GenerationContext, derived: String, api: String): String {
    ...
    })${superUnit().isNotEMPTY().then { (superUnit() as ConstructorI).toTypeScriptCall(c, (parent() != superUnit().parent()).ifElse("super", "this")) }} ${
...
fun <T : AttributeI> T.toTypeScriptAssign(c: GenerationContext): String =
        "this.${name()} = ${name()}"
...
fun <T : AttributeI> T.toTypeScriptType(c: GenerationContext, derived: String): String = type().toTypeScript(c, derived, this)
