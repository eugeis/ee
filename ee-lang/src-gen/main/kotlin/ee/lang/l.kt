...
object l : StructureUnit({ namespace("ee.lang") }) {
    ...
    val Composite = CompilationUnit({ derivedFrom(MultiMapHolder) })
    ...
    val MultiListHolder = CompilationUnit({ derivedFrom(MultiHolder) })
    val MultiMapHolder = CompilationUnit({ derivedFrom(MultiHolder) })
