...
fun <T : Attribute> T.toSqlFkColumnName(context: SqlContext, relationUniqueType: Boolean = true): String {
    ...
    if (complexType is Entity) {
        return relationUniqueType.ifElse({ complexType.toSqlFkColumnName(context) },
            { "${toSql(context)}_${complexType._id?.toSql(context)}}" })
    } else {
...
fun <T : Entity> T.toSqlFkColumnDef(context: SqlContext, indent: String = "", namePrefix: String = "",
    commentPrefix: String = ""): String {
    return "${_id?.toSqlColumnDef(context, indent, namePrefix, commentPrefix)}"
}

fun <T : Attribute> T.toSqlColumnDef(context: SqlContext, indent: String = "", namePrefix: String = "",
    commentPrefix: String = "", relationUniqueType: Boolean = true): String {
    val complexType = type
    if (complexType is Basic) {
        return "${toSqlComment(context, indent, commentPrefix)}${complexType.toSqlColumnDef(context, indent,
            toSqlColumnNamePrefix(context), "$commentPrefix$name.")}"
    } else if (complexType is Entity) {
        return "${toSqlComment(context, indent, commentPrefix)}${complexType.toSqlFkColumnDef(context, indent,
            relationUniqueType.ifElse({ complexType.toSqlColumnNamePrefix(context) },
                { toSqlColumnNamePrefix(context) }), "$commentPrefix$name.")}"
    } else {
        return "$indent$namePrefix${toSql(context)}${toSqlTypeDef(context)}${toSqlComment(context, indent,
            commentPrefix)}"
    }
}

fun List<Attribute>.toSqlColumnDef(context: SqlContext, indent: String, namePrefix: String = "",
    commentPrefix: String = ""): String {
    return "${joinToString(",$nL") { it.toSqlColumnDef(context, indent, namePrefix, commentPrefix) }}"
}

fun <T : Attribute> T.toSqlRelationTableName(context: SqlContext, parent: Entity,
    relationUniqueType: Boolean = true): String {
...
fun <T : Attribute> T.toSqlCreateRelationTable(context: SqlContext, indent: String = "", parent: Entity): String {
    ...
    return return """$nL${toSqlComment(context, indent, "",
        parent)}${indent}CREATE TABLE IF NOT EXISTS ${toSqlRelationTableName(context, parent)} (
...
fun <T : Attribute> T.toSqlDropRelationTable(context: SqlContext, indent: String = "", parent: Entity): String {
    return """$nL${toSqlComment(context, indent, "", parent)}${indent}DROP TABLE IF EXISTS ${toSqlRelationTableName(
        context, parent)};
"""
}
...
fun Entity.toSqlCreateRelationTables(context: SqlContext, indent: String = ""): String {
    if (virtual) return ""
    return """${propsAll.filter { it.type.multi() }.joinToString("${nL}") {
        it.toSqlCreateRelationTable(context, indent, this)
    }}"""
}
...
fun Entity.toSqlCreateForeignKeys(context: SqlContext, indent: String = ""): String {
    if (virtual) return ""
    return """${propsAll.filter { !it.type.multi() && it.type is Entity }.joinToString(
        "${nL}") { it.toSqlCreateForeignKey(context, indent, this) }}"""
}
...
fun CompilationUnit.toSqlColumnDef(context: SqlContext, indent: String = "", namePrefix: String = "",
    commentPrefix: String = ""): String {
...
fun Entity.toSqlCreateIndexes(context: SqlContext, indent: String = ""): String {
    if (virtual) return ""
    return """${propsAll.filter { !it.type.multi() && it.type is EnumType }.joinToString("${nL}") {
        it.toSqlCreateIndex(context, indent, this)
    }}"""
}
...
fun Entity.toSqlDropRelationTables(context: SqlContext, indent: String = ""): String {
    if (virtual) return ""
    return """${propsAll.filter { it.type.multi() }.joinToString("${nL}") {
        it.toSqlDropRelationTable(context, indent, this)
    }}"""
}
