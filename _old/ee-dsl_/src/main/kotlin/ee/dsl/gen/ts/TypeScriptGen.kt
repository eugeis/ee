...
class TypeScriptContext : GenerationContext {
    ...
    private fun toImports(indent: String): String {
        return types.isEmpty().ifElse("", {
            types.filter { it.namespace.isNotEmpty() && it.namespace != namespace }.map { "${it.namespace}.${it.name};" }.sorted().
                    joinToString(nL, indent)
        })
    }
}
...
fun <T : Literal> T.toTypeScriptLiteral(context: TypeScriptContext, indent: String = "", mappings: T.(context: TypeScriptContext, indent: String) -> String = { c, ind -> "" }): String {
    return """${toTypeScriptComment(context, indent)}${mappings(context, indent)}$indent${name.toUnderscoredUpperCase()}"""
}

fun <T : Attribute> T.toTypeScriptMember(context: TypeScriptContext, indent: String = "", mappings: T.(context: TypeScriptContext, indent: String) -> String = { c, ind -> "" }, modifier: String = ""): String {
    return """${toTypeScriptComment(context, indent)}${mappings(context, indent)}$indent$modifier$name: ${type.toTypeScript(context)};"""
}

fun <T : Attribute> T.toTypeScriptGetterImpl(context: TypeScriptContext, indent: String = "", mappings: T.(context: TypeScriptContext, indent: String) -> String = { c, ind -> "" }, modifier: String = ""): String {
    val newIndent = "$indent${tab}"
    return """${toTypeScriptComment(context, indent)}${mappings(context, indent)}$indent$modifier$name(): ${type.toTypeScript(context)} {
...
fun <T : Attribute> T.toTypeScriptSetterImpl(context: TypeScriptContext, indent: String = "", mappings: T.(context: TypeScriptContext, indent: String) -> String = { c, ind -> "" }, modifier: String = ""): String {
    val newIndent = "$indent${tab}"
    return """${toTypeScriptComment(context, indent)}${mappings(context, indent)}$indent$modifier$name($type.to name: ${type.toTypeScript(context)}) {
...
fun <T : Attribute> T.toTypeScriptSignature(context: TypeScriptContext, indent: String = "", mappings: T.(context: TypeScriptContext, indent: String) -> String = { c, ind -> "" }): String {
    return "${toTypeScriptComment(context, indent)}${mappings(context, indent)}$indent$name: ${type.toTypeScript(context)}"
}
...
fun <T : Operation> T.toTypeScriptIfc(context: TypeScriptContext, indent: String = "", mappings: T.(context: TypeScriptContext, indent: String) -> String = { c, ind -> "" }): String {
    return "${toTypeScriptComment(context, indent)}${mappings(context, indent)}$indent$name(${params.toTypeScriptSignature(context, indent)}): ${ret?.toTypeScriptSignatureReturn(context)};"
}

fun <T : Operation> T.toTypeScriptImpl(context: TypeScriptContext, indent: String = "", mappings: T.(context: TypeScriptContext, indent: String) -> String = { c, ind -> "" }, modifier: String = ""): String {
    return """${toTypeScriptComment(context, indent)}${mappings(context, indent)}$indent$modifier$name(${params.toTypeScriptSignature(context, indent)}): ${ret?.toTypeScriptSignatureReturn(context)} {
${ret?.toTypeScriptReturnDefault(context, indent)}
$indent}
"""
}

fun CompilationUnitD.toTypeScriptIfc(context: TypeScriptContext, indent: String = "", derived: TypeDerived<CompilationUnitD> = api): String {
    val newIndent = "$indent${tab}"
...
fun CompilationUnitD.toTypeScriptImpl(context: TypeScriptContext, indent: String = "", derived: TypeDerived<CompilationUnitD> = api): String {
    val newIndent = "$indent${tab}"
...
fun EnumTypeD.toTypeScriptEnum(context: TypeScriptContext, indent: String = "", derived: TypeDerived<EnumTypeD> = enum): String {
    val newIndent = "$indent${tab}"
