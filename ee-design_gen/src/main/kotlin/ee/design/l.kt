...
object l : Composite({ namespace("ee.design") }) {
    ...
    object Module : Composite({ derivedFrom(StructureUnit) }) {
        val parentNamespace = AttributeI({ type(n.Boolean).value(false) })
        val dependencies = AttributeI({ type(Module).multi(true) })
    }

    object ExternalModule : Composite({ derivedFrom(Module) }) {
        val externalTypes = AttributeI({ type(l.ExternalType).multi(true).nonFluent("type") })
    }

    object ModuleGroup : Composite({ derivedFrom(StructureUnit) }) {
        val modules = AttributeI({ type(Module).multi(true) })

    }

    object Comp : Composite({ derivedFrom(ModuleGroup) }) {
        //e.g release units or sub components
        val moduleGroups = AttributeI({ type(ModuleGroup).multi(true) })
    }

    object Model : Composite({ derivedFrom(StructureUnit) }) {
        val models = AttributeI({ type(Model).multi(true) })
        val comps = AttributeI({ type(Comp).multi(true) })
    }

    object Bundle : Composite({ derivedFrom(StructureUnit) }) {
        val units = AttributeI({ type(StructureUnit).multi(true) })
    }

    object Facet : Composite({ derivedFrom(ModuleGroup) })
...
object n : Composite({ namespace("ee.design").name("native") }) {
    val Void = Item()
    val Any = Item()
    val Path = Item()
    val Text = Item()
