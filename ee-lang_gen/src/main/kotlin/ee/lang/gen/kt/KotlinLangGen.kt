...
fun <T : ItemIB<*>> T.toKotlinEMPTY(c: GenerationContext, derived: String): String {
...
fun <T : AttributeI> T.toKotlinTypeSingle(c: GenerationContext, api: String): String {
    return type().isNative().ifElse({ c.n(type(), api) }, { "${c.n(type(), api)}B<*>" })
}
...
fun <T : AttributeI> T.toKotlinDslBuilderMethods(c: GenerationContext, derived: String, api: String, parent: ItemIB<*> = parent()): String {
    val value = (name() == "value").ifElse("aValue", "value")
...
fun <T : CompositeIB<*>> T.toKotlinDslBuilderI(c: GenerationContext, api: String = DerivedNames.API): String {
...
open class ${c.n(this, derived)}B<B : ${c.n(this, api)}B<B>>(value: B.() -> Unit = {}) : ${c.n(derivedFrom(), derived)}B<B>(value)${(derived != api).then(
            { ", ${c.n(this, DerivedNames.API)}B<B>" })} {${
...
fun <T : ItemIB<*>> T.toKotlinObjectTreeCompilationUnit(c: GenerationContext, derived: String = DerivedNames.DSL_TYPE): String {
    return """    val ${c.n(this, derived)} = ${c.n(l.CompilationUnit, derived)}${derivedFrom().isNotEMPTY().then {
        " { derivedFrom(${c.n(derivedFrom(), derived)}) }"
    }}"""
}

fun <T : CompositeIB<*>> T.toKotlinDslObjectTree(c: GenerationContext, derived: String = DerivedNames.DSL_TYPE): String {
    return """
