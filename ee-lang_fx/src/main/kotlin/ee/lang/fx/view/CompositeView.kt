...
class CompositeView() : View("Explorer") {
    ...
    override val root = accordion(*model.elementGroups.map { group ->
        titledpane(group.first, treeview<ItemNode> {
            ...
            onUserSelect {
                tasksController.onStructureUnitsSelected(selectionModel.selectedItems.map { it.value.el })
            }
            populate {
                it.value.factory()
            }
...
open class ExplorerModel(val name: String, val elementGroups: List<Pair<String, MultiHolderI<*, *>>>,
    val nodeFilter: (ItemI<*>) -> Boolean)
