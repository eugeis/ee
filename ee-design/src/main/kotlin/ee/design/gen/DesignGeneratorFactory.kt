...
open class DesignGeneratorFactory : LangGeneratorFactory {
    ...
    open fun eventDrivenGo(fileNamePrefix: String = ""): GeneratorGroupI<StructureUnitI<*>> {
        ...
        val ret = GeneratorGroup("eventDrivenGo", listOf(GeneratorGroupItems("swaggerModule", items = modules, generators = moduleGenerators),
                Generator("swaggerComponent", contextBuilder = swaggerContextBuilder, items = components,
                        templates = { listOf(swaggerTemplates.model()) })))

        moduleGenerators.addAll(listOf(GeneratorSimple("ApiBase", contextBuilder = goContextBuilder,
    ...
    open fun angular(fileNamePrefix: String = ""): GeneratorGroupI<StructureUnitI<*>> {
        ...
        val moduleGenerators = arrayListOf<GeneratorI<StructureUnitI<*>>>()
        val ret = GeneratorGroup("angular", listOf(GeneratorGroupItems("angularModules", items = modules, generators = moduleGenerators)))
