...
import ee.lang.gen.swagger.toSwagger
...
fun <T : CompI> T.toSwagger(c: GenerationContext): String {
    val moduleEntities = findDownByType(EntityI::class.java).filter { !it.virtual() && it.derivedAsType().isEmpty() }
            .groupBy { it.findParentMust(ModuleI::class.java) }
...
produces:
  - application/json
paths:${moduleEntities.forEach { module, items ->
        items.forEach { entity ->
            """
    /${module.name().decapitalize()}/${entity.name().decapitalize()}:
"""
        }
    }}
parameters:
responses:
definitions:${moduleEntities.forEach { module, items -> items.forEach { entity -> entity.toSwagger(c) } }}
    """
}